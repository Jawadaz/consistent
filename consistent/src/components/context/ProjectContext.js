...
export const ProjectContextProvider = ({ children }) => {
  ...
  const updateHistoryIndex = (index) => {
    ...
    setIsUndoDisabled(
      index === projectCellsHistory.length - 1 || isProjectLocked
    );
  ...
  const saveAsClick = (id) => {
    ...
    var blob = new Blob(
      [JSON.stringify({ ...projectData, cells: projectCells })],
    ...
    saveAs(
      ...
      `${
        projectData.title
      } v${new Date().toLocaleDateString()}-${new Date().toLocaleTimeString()}.json`
    );
  };
  ...
  const updateCell = (id, updatedCell) => {
    ...
    updateProjectCells(
      projectCells.map((cell) =>
        cell.id === id ? { ...cell, ...updatedCell } : cell
      )
    );
  };
  ...
  const updateCellContent = (id, content) => {
    ...
    updateProjectCells(
      projectCells.map((cell) =>
        cell.id === id ? { ...cell, content: content } : cell
      )
    );
  ...
  const updateCellTags = (id, tags) => {
    ...
    updateProjectCells(
      projectCells.map((cell) =>
        cell.id === id ? { ...cell, tags: tags } : cell
      )
    );
  };
  ...
  useEffect(() => {
    ...
    const updateProjectTags = () => {
      //
      const allTags = projectCells.map((cell) => cell.tags).flat();
